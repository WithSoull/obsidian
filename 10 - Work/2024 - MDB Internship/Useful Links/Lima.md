`brew install lima` <- установил (еще подтянуть qemu `brew install qemu`)

`~/.lima/default/lima.yaml` <- в этот файлик пишет это
...
`limactl create --arch=x86_64 --cpus=2 --memory=2` <- создать виртуалку

`limactl start default` <- запустить виртуалку

`limactl stop default `<- остановить виртуалку


Чтобы установить докер на ВМ -> https://docs.docker.com/engine/install/ubuntu/
