...
Но этой пары ярлыков недостаточно. Отдельно полезно различать архитектурные режимы:
...
- они объясняют не только *куда* мы растем, но и *как устроены compute, storage и coordination*.

Отсюда еще один практический вывод:
- обычно не стоит проектировать систему больше чем на один порядок нагрузки вперед;
- архитектура под `10x` рост может быть разумной;
- архитектура под гипотетические `100x` часто уже превращается в premature optimization.
