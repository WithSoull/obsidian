...
Для этого use-case важно понимать, что нельзя несколько раз закрывать канал, поэтому если требуется несколько раз передать сигнал, нужно использовать [[30 - Learning/20 - Tech Stack/Golang/2. Concurrancy/Synchronization primitives/Cond|Cond variable]].